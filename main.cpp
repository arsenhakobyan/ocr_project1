...
void show_help_and_exit(const char *cmd)
{
    ...
    cout << "    Default classifier files (../res/trained_classifierNM*.xml) must be in current directory" << endl << endl;
    exit(-1);
}
...
void textRecognition(Mat &src, vector<Rect> &groups) 
{
    for (int i=(int)groups.size()-1; i>=0; i--) {
        const string name = to_string(groups.at(i).x) + "_" +  to_string(groups.at(i).y) + ".jpg"; // TODO Change this to the temprorary image.
        imwrite(name, src(groups.at(i)).clone());
        system_exec("convert " + name + " -resize 800 -sharpen 0x7" + name, "Error in the conversion phase.");
...
void groups_draw(Mat &src, vector<Rect> &groups) {
    ...
    {
        if (src.type() == CV_8UC3) {
            rectangle(src,groups.at(i).tl(),groups.at(i).br(),Scalar( 0, 255, 255 ), 1, 8 );
...
int main(int argc, const char * argv[])
{
    ...
    Ptr<ERFilter> er_filter1 = createERFilterNM1(loadClassifierNM1("../res/trained_classifierNM1.xml"), 3, 0.0001f, 0.4f, 0.55f, true, 0.1f);
    Ptr<ERFilter> er_filter2 = createERFilterNM2(loadClassifierNM2("../res/trained_classifierNM2.xml"), 0.78);
    ...
    textRecognition(src, groups_boxes);
    //imshow("grouping",src);
